...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE profiles
      ADD CONSTRAINT profiles_contest_count_check
...
CREATE POLICY "contest_matches_select" ON contest_matches
  FOR SELECT TO authenticated USING (true);
CREATE POLICY "contest_matches_insert" ON contest_matches
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = author_id);
CREATE POLICY "contest_matches_update" ON contest_matches
  FOR UPDATE TO authenticated USING (auth.uid() = author_id);
CREATE POLICY "contest_matches_delete" ON contest_matches
...
CREATE POLICY "contest_apps_select" ON contest_applications
  FOR SELECT TO authenticated
  USING (
    applicant_id = auth.uid() OR
    contest_match_id IN (SELECT id FROM contest_matches WHERE author_id = auth.uid())
  );
CREATE POLICY "contest_apps_insert" ON contest_applications
  FOR INSERT TO authenticated WITH CHECK (applicant_id = auth.uid());
CREATE POLICY "contest_apps_update" ON contest_applications
...
CREATE POLICY "contest_chat_rooms_select" ON contest_chat_rooms
  FOR SELECT TO authenticated
  USING (id IN (SELECT room_id FROM contest_chat_members WHERE user_id = auth.uid()));
CREATE POLICY "contest_chat_rooms_insert" ON contest_chat_rooms
...
CREATE POLICY "contest_chat_members_select" ON contest_chat_members
  FOR SELECT TO authenticated
  USING (room_id IN (SELECT room_id FROM contest_chat_members WHERE user_id = auth.uid()));
CREATE POLICY "contest_chat_members_insert" ON contest_chat_members
...
CREATE POLICY "contest_msgs_select" ON contest_chat_messages
  FOR SELECT TO authenticated
  USING (room_id IN (SELECT room_id FROM contest_chat_members WHERE user_id = auth.uid()));
CREATE POLICY "contest_msgs_insert" ON contest_chat_messages
  FOR INSERT TO authenticated
  WITH CHECK (
